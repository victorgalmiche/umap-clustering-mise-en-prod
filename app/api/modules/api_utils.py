...
def _get_polars_from_request(content: bytes) -> pl.DataFrame:
    ...
    if df.width < 3:
        raise HTTPException(status_code=400, detail="CSV file must have at least 3 numerical columns.")
    return df
...
def fit_umap_model(df: pl.DataFrame, params: dict, tracker: ExperimentTracker) -> Tuple[Any, StandardScaler, Any, Any]:
    """
...
def prepare_umap_params(df: pl.DataFrame, params: UmapParameters) -> tuple[dict, int, int]:
    ...
    umap_params.update({"n_samples": n_samples, "n_features": n_features})
