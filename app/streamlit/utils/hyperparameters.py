...
def select_umap_params(n_valid_columns):
    ...
    params["n_components"] = st.sidebar.slider("n_components", 2, min(n_valid_columns, 10), key="n_components")
    params["n_neighbors"] = st.sidebar.slider("n_neighbors", 2, 100, key="n_neighbors")
    params["n_epochs"] = st.sidebar.slider("n_epochs", 10, 300, key="n_epochs")
    params["min_dist"] = st.sidebar.slider("min_dist", 0.01, 1.0, key="min_dist")
    params["knn_metric"] = st.sidebar.selectbox(
        "KNN metric", ["euclidean", "chebyshev", "minkowski"], index=0, key="knn_metric"
    )
    params["knn_method"] = st.sidebar.selectbox("KNN method", ["exact", "approx"], index=0, key="knn_method")

    return params


def select_clustering_method(key_suffix):
    return st.selectbox("Method", ["KMeans", "HDBSCAN"], key="clusering_method" + key_suffix)
