...
target_names = (
    fashion.target_names
    if hasattr(fashion, "target_names")
...
for i, (name, algo) in enumerate(algorithms.items()):
    ...
    ax = axes[i]
    scatter = ax.scatter(
        embedding[:, 0], embedding[:, 1], c=y, cmap="Spectral", s=5, alpha=0.7
    )
...
plt.suptitle(
    f"Qualitative Comparison: Fashion-MNIST Embeddings ({n_points} points)", fontsize=18
)
